...
移除停用词的工作是由 `stop` 停用词标记过滤器完成的，可以通过创建自定义的分析器来使用它（参见 使用停用词标记分析器{ref}/analysis-stop-tokenfilter.html [`stop` 停用词标记过滤器])。但是，也有一些自带的分析器预置使用停用词标记分析器：
...
{ref}/analysis-pattern-analyzer.html[`pattern` 模式分析器]::
    ...
    默默认使用空的停用词列表：为`_none_`，与 `standard`分析器类似。
...
TIP: 
Elasticsearch中预定义的与语言相关的停用词列表可以在文档(("languages", "predefined stopword lists for"))) {ref}/analysis-stop-tokenfilter.html[`stop` token filter] 中找到。

停用词可以通过指定一个特殊列表 `__none_`_ 来禁用。例如，使用 `_english`_ 分析器而不使用停用词，可以通过以下方式做到：
