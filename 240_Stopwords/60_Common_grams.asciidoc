[[common-grams]]
=== _common_grams_ 标记过滤器（common_grams Token Filter）

 `common_grams` 标记过滤器是针对短语查询能更高效的使用停用词而设计的。 ((("stopwords", "phrase queries and", "common_grams token filter")))((("common_grams token filter")))((("phrase matching", "stopwords and", "common_grams token filter")))它与 shingles 标记过滤器((("shingles", "shingles token filter")))类似（参见 查找相关词（<<shingles>>)), 为每个相邻词对生成 ((("bigrams")))，用示例解释更为容易。
...
<1> 首先我们基于 `common_grams` 标记过滤器创建两个标记过滤器： 
...
The positions data for  is much smaller than for , so it is
...
那么 `search_grams` 分析器会输出单个标记：`the_quick`。这将原来昂贵的查询（查询 `the` 和 `quick`）转换成了对单个词项的高效查找。
